...
const User_Profile_Form = (props) =>{
    const [editable_usr_id, setEditable_usr_id]=useState('');
    ...
    useEffect(() => {
        const data_innfo={
            dd_up: 'get',
            dd_up_ID: authCtx.token,
        }
        // console.log(data_innfo);
        axios({
            method: "post",
            url: process.env.REACT_APP_LOGIN_USERS_PROFILE_INFO,
            headers: { "content-type": "application/json" },
            data: data_innfo
        })
        .then(result => {
            setRows(result.data);
            setEditable_usr_id(result.data[0].row_id);
        })
    }, [])
                        ...
                        <Box sx={{ width: '100%', typography: 'body1' }}>
                            <TabContext value={value}>
                                ...
                                <TabPanel className={classes.emp_frm_tab_cnt} value="Personal">
                                    {editable_usr_id && <Personal_info editable_id={editable_usr_id} />}
                                    {editable_usr_id && <Contact_info editable_id={editable_usr_id} />}
                                    {editable_usr_id && <Address_info editable_id={editable_usr_id}/>}
                                    {editable_usr_id && <Family_Members_Table editable_id={editable_usr_id} />}
                                    {editable_usr_id && <Emegency_Contact_Table editable_id={editable_usr_id} />}
                                </TabPanel>
                                <TabPanel className={classes.emp_frm_tab_cnt} value="Work">
                                    {editable_usr_id && <Work_basic_info editable_id={editable_usr_id} />}
                                    {editable_usr_id && <Emp_work_info editable_id={editable_usr_id} />}
                                    {editable_usr_id && <Emp_work_history_Table editable_id={editable_usr_id} />}
                                    {editable_usr_id && <Team_repoting_manager editable_id={editable_usr_id} />}
                                    {editable_usr_id && <Direct_Report_Table editable_id={editable_usr_id} />}
                                </TabPanel>
                                ...
                                <TabPanel className={classes.emp_frm_tab_cnt} value="Education">
                                    {editable_usr_id && <Education_Info_Table editable_id={editable_usr_id} />}
                                </TabPanel>
                                ...
                                <TabPanel className={classes.emp_frm_tab_cnt} value="Documents">
                                    {editable_usr_id && <Documents_main editable_id={editable_usr_id} />}
                                </TabPanel>
