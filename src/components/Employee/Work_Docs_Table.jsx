...
import Work_Docs_Form from './Work_Docs_Form';
...
const Work_Docs_Table = (props) =>{
    ...
    const [ModelTitle, SetModelTitle]=useState('Add Work');
    ...
    useEffect(() => {
        ...
        axios({
            method: "post",
            url: process.env.REACT_APP_USERS_WORK_DOCS_LIST_INFO,
    ...
    const deleteSelected =(e) =>{
        ...
        .then((willDelete) => { 
            ...
            const data_innfo={
              ...
              axios({
                method: "post",
                url: process.env.REACT_APP_USERS_WORK_DOCS_DLT_INFO,
                headers: { "content-type": "application/json" },
                data: data_innfo
            })
            .then(result => {
    ...
    return (
        <section>
            {modelshow && <Model title={ModelTitle} onConfirm={() => {setModelShow(false)}}><Work_Docs_Form editedID={props.editable_id} editedTableID={editedRowId} onAddRow={() => {setModelShow(false)}} onChange={() => setDataCanged(true)} /></Model>}
                                ...
                                {rows && rows.length > 0 && rows.map((row) => {
                                    ...
                                    <tr id={row.r_id} key={row.r_id}>
                                        ...
                                        <td className='align_center'>
                                            <button style={{margin: '0px'}} type="button" onClick={() => {setImagePopUpEnditedIdHander(row.work_file); SetModelTitle(row.type_v)}} className='color_gry font_15'><AiFillEye /></button>
...
export default Work_Docs_Table;