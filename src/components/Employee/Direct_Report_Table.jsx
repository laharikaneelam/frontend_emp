...
import Direct_Report_Form from './Direct_Report_Form';
...
const Direct_Report_Table = (props) =>{
    const [modelshow, setModelShow]=useState(false);
    const [editedRowId, setEditedRowId]=useState('');
    const [rows, setRows]=useState('');
    const [ModelTitle, SetModelTitle]=useState('Add Direct Reports');
    ...
    useEffect(() => {
        ...
        axios({
            method: "post",
            url: process.env.REACT_APP_DIRECT_REPORT_LIST,
    ...
    const deleteSelected =(e) =>{
        ...
        .then((willDelete) => { 
            ...
            const data_innfo={
              ...
              axios({
                method: "post",
                url: process.env.REACT_APP_DIRECT_REPORT_DLT,
                headers: { "content-type": "application/json" },
                data: data_innfo
            })
            .then(result => {
    ...
    return (
        <section>
            {modelshow && <Model title={ModelTitle} onConfirm={() => {setModelShow(false)}}><Direct_Report_Form editedID={props.editable_id} editedTableID={editedRowId} onAddRow={() => {setModelShow(false)}} onChange={() => setDataCanged(true)} /></Model>}
            <header>
                Direct Reports
                            ...
                            <tbody>
                                {rows && rows.length > 0 && rows.map((row) => {
                                    ...
                                    <tr id={row.r_id} key={row.r_id}>
                                        <td>
                                            {/* {row.Name} */}
                                            {idV.length > 0 && idV.filter((ocuntry) => ocuntry.vl === row.report_to).map( (getcountry)=>(
                                        ...
                                        <td className='align_center'>
                                            <button style={{margin: '0px'}} type="button" onClick={() => {setEnditedIdHander(row); SetModelTitle('Update Direct Reports')}} className='color_gry font_15'><MdModeEdit /></button>
...
export default Direct_Report_Table;