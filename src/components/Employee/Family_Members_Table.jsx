...
import Family_members_From from './Family_members_From';
...
const Family_Members_Table = (props) =>{
    // console.log(props.editable_id);
    const [modelshow, setModelShow]=useState(false);
    const [editedRowId, setEditedRowId]=useState('');
    const [rows, setRows]=useState('');
    const [ModelTitle, SetModelTitle]=useState('Add Family Info');
    ...
    useEffect(() => {
        ...
        axios({
            method: "post",
            url: process.env.REACT_APP_FAMILY_INFO_LIST,
    ...
    const deleteSelected =(e) =>{
        ...
        .then((willDelete) => { 
            ...
            const data_innfo={
              ...
              axios({
                method: "post",
                url: process.env.REACT_APP_FAMILY_INFO_DLT,
                headers: { "content-type": "application/json" },
                data: data_innfo
            })
            .then(result => {
    ...
    return (
        <section>
            {modelshow && <Model title={ModelTitle} onConfirm={() => {setModelShow(false)}}><Family_members_From editedID={props.editable_id} editedTableID={editedRowId} onAddRow={() => {setModelShow(false)}} onChange={() => setDataCanged(true)} /></Model>}
            <header>
                Family Members
                            ...
                            <tbody>
                                {rows && rows.length > 0 && rows.map((row) => {
                                    ...
                                    <tr id={row.r_id} key={row.r_id}>
                                        ...
                                        <td>{row.relation}</td>
                                        <td>{row.dob}</td>
                                        <td>{row.dependent==='Y' ? 'Yes' : 'No'}</td>
                                        <td className='align_center'>
                                            <button style={{margin: '0px'}} type="button" onClick={() => {setEnditedIdHander(row); SetModelTitle('Update Family Info')}} className='color_gry font_15'><MdModeEdit /></button>
...
export default Family_Members_Table;