...
import Emp_work_history from './Emp_work_history';
...
const Emp_work_history_Table = (props) =>{
    const [modelshow, setModelShow]=useState(false);
    const [editedRowId, setEditedRowId]=useState('');
    const [rows, setRows]=useState('');
    const [ModelTitle, SetModelTitle]=useState('Add Work History');
    ...
    useEffect(() => {
        ...
        axios({
            method: "post",
            url: process.env.REACT_APP_WRK_HSRT_LIST,
    ...
    const deleteSelected =(e) =>{
        ...
        .then((willDelete) => { 
            ...
            const data_innfo={
              ...
              axios({
                method: "post",
                url: process.env.REACT_APP_WRK_HSRT_DELETE,
                headers: { "content-type": "application/json" },
                data: data_innfo
            })
            .then(result => {
    ...
    return (
        <section>
            {modelshow && <Model title={ModelTitle} onConfirm={() => {setModelShow(false)}}><Emp_work_history editedID={props.editable_id} editedTableID={editedRowId} onAddRow={() => {setModelShow(false)}} onChange={() => setDataCanged(true)} /></Model>}
            <header>
                Work History
...
export default Emp_work_history_Table;