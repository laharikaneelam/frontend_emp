...
const initialState = {
  ...
  const reducer = (state, action) => {
    ...
    switch (action.type) {
        case 'SET_WORK_DESCR':
        return { ...state, work_descr: action.payload };
        case 'SET_VERIFICATION':
        return { ...state, verification: action.payload };
        case 'SET_CERT_TITLE':
        return { ...state, cert_title: action.payload };
        case 'SET_FILE_ID':
...
const Work_Docs_Form = (props) =>{
    ...
    useEffect(() => {
        if(props.editedTableID!==''){
            ...
            dispatch({ type: `SET_WORK_DESCR`, payload: props.editedTableID['work_descr'] });
            dispatch({ type: `SET_VERIFICATION`, payload: props.editedTableID['verification'] });
            dispatch({ type: `SET_CERT_TITLE`, payload: props.editedTableID['type_v'] });
    ...
    const submitHandler =(event) =>{
        ...
        // if(row_id!==''){
        //     if(state.work_descr.length > 0 && logo_img!==''){
        // }else{
        //     if(state.work_descr.length > 0 && verification!==''){
        // }
        if(state.work_descr.length > 0){
            const data_innfo={
                // first_name: state.name,
                // ac_type: state.ac_type,
                R_ID: row_id,
                file_id : logo_img,
                work_descr : state.work_descr,
                verification : state.verification,
                cert_title : state.cert_title,
            ...
            axios({
                method: "post",
                url: process.env.REACT_APP_USERS_WORK_DOCS_INFO,
                        ...
                        <div className={classes.emp_frm_tab_cnt_val}>
                            <div className="row">
                                <div className="col-sm-4">
                                    <label htmlFor="">Title <span className='imp_clss'>*</span></label>
                                </div>
                                <div className="col-sm-8">
                                    <Input className="width_100 mt-2" defaultValue="" name="cert_title" value={state.cert_title} onChange={handleInputChange}  />
                                    {state.errors.cert_title && (
                                        <span className="error">{state.errors.cert_title}</span>
                                    )}
                                </div>
                            </div>
                            <div className="row">
                                <div className="col-sm-4">
                                    <label htmlFor="">Description <span className='imp_clss'>*</span></label>
                                </div>
                                <div className="col-sm-8">
                                    <Input className="width_100 mt-2" defaultValue="" name="work_descr" value={state.work_descr} onChange={handleInputChange}  />
                                    {state.errors.work_descr && (
                                        <span className="error">{state.errors.work_descr}</span>
                                    )}
                                </div>
                            </div>
                            {row_id && <div className="row mt-4"> 
                                <div className="col-sm-4">
                                    <label htmlFor="">Verification<span className='imp_clss'>*</span></label>
                                </div>
                                <div className="col-sm-8 mt-2">
                                    <label className="mt-2" style={{display: 'inline'}} htmlFor=""><input type="radio" name="verification" value="Verified" checked={state.verification==='Verified'} onChange={handleInputChange} /> Verified</label>
                                    <label className="mt-2" style={{marginLeft : '15px', display: 'inline'}} htmlFor=""><input type="radio" name="verification" checked={state.verification==='Not Verified'}  value="Not Verified" onChange={handleInputChange} /> Not Verified</label>
                                </div>
                            </div>}
